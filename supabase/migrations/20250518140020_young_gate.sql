/*
  ...
  1. Changes
    - Creates personas table with initial data
    - Modifies chat_sessions to use UUID for persona references
    - Updates all related foreign key constraints
    - Preserves existing chat data
  
  2. Security
    - Enables RLS on personas table
    - Recreates all necessary policies
*/
...
CREATE POLICY "Users can view messages from their chats"
  ...
  FOR SELECT
  TO public
  USING (chat_id IN (SELECT id FROM chat_sessions WHERE user_id = auth.uid()));

CREATE POLICY "Users can create messages in their chats"
  ON messages
  FOR INSERT
  TO public
  WITH CHECK (chat_id IN (SELECT id FROM chat_sessions WHERE user_id = auth.uid()));
